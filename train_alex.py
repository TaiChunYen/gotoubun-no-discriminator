...
alexnet = models.alexnet(num_classes=opt.n_classes).to(device)
...
for epoch in range(opt.n_epochs):
    ...
    for i, (imgs, labels) in enumerate(data_loader):
        ...
        outputs = alexnet(imgs.to(device))
    ...
    for j, (t_imgs, t_labels) in enumerate(test_loader):
        ...
        t_total = 0
        t_correct = 0
        t_test = alexnet(t_imgs.to(device))
    ...
    if epoch%10==9:
        torch.save(alexnet.state_dict(),'./a_model-%s.pkl'%str(epoch+1))
        
    

print('Finished Training')

torch.save(alexnet.state_dict(),'./a_model-last.pkl')
